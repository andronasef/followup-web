...
**Domain:** Self-hosted, anonymous, one-on-one web chat (PWA) with mandatory web push and two-way LLM translation
**Researched:** 2026-07-20
**Confidence:** HIGH on versions and Next.js/SSE mechanics; MEDIUM on Coolify-specific proxy behaviour (community-sourced, not vendor-documented)

> Scope note: the locked decisions in `.planning/PROJECT.md` are treated as given. This document pins versions, names the integration traps, and flags **two** locked choices that are wrong-ish in 2026 (bcrypt, and `pg` as the LISTEN driver). Everything else in the locked stack is sound.
...
npm install jose@6.2.3 @node-rs/argon2@2.0.2 web-push@3.6.7 openai@6.48.0
...
npm install -D typescript@6.0.3 @types/node @types/react @types/react-dom \
              @types/web-push drizzle-kit@0.31.10 tailwindcss@4.3.3 @tailwindcss/postcss
```

> Do **not** run `npm install typescript` without a version — you will silently get 7.0.2 and Next 16.2 will fail to detect TypeScript.
...
**The Dockerfile shape** (verified against Next 16.2.9 docs and the generated `server.js` source):
...
**Coolify ↔ Postgres networking — the #1 self-host trap:**

A Coolify **application** and a Coolify **database** are placed on *different* Docker networks by default. Connecting via the "Postgres URL (internal)" shown in the DB UI will fail with DNS resolution errors until you enable **Advanced → Connect To Predefined Network** on the application. Then use the internal URL verbatim (hostname is the DB container's UUID-ish name, port 5432). Do not expose Postgres publicly — internal-only is both the secure and the low-latency option, and `LISTEN/NOTIFY` latency matters here. Known secondary issue: a container attached to *both* the project network and the shared `coolify` network can get non-deterministic DNS resolution — keep the app on one network.

### 2. SSE in Next.js 16 App Router route handlers

**What actually works** (verified against Next 16.2.9 docs + source):

```ts
// app/api/stream/route.ts
export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'   // NOT optional — see below
...
export async function GET(req: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const cleanup = () => { clearInterval(heartbeat); /* unlisten */ try { controller.close() } catch {} }
...
- **Traefik (Coolify's default proxy) is the risk.** There are open Coolify reports of SSE/WebSocket connections being unreliable behind Traefik in production while working in plain Docker Compose (coollabsio/coolify#4002 — still open, no confirmed fix), plus a Traefik 3.6 long-connection 408-after-~5-minutes class of bug (coollabsio/coolify#8298). **Design for the stream dying.** Concretely:
  ...
  - Deliberately close the stream server-side after ~4 minutes and let `EventSource` auto-reconnect. A short, expected reconnect is far more robust than fighting proxy timeouts.
  - Emit `id:` on every message and honour `Last-Event-ID` on reconnect, replaying from the DB. This makes "no message is ever lost" true regardless of proxy behaviour.
  - Client `EventSource` reconnects automatically on network close but **not** on HTTP error status — add an `onerror` handler with your own backoff.
...
The trap you named is real: **`LISTEN` is connection-scoped session state.** A pooled client that issues `LISTEN` will silently stop receiving notifications the moment the pool hands that socket to someone else or recycles it. You need a dedicated, never-pooled connection, plus reconnect logic, plus a "what did I miss while disconnected?" catch-up query.

`pg` gives you none of that — you hand-roll a `pg.Client` (not `Pool`), an `error`/`end` handler, exponential backoff, re-issue of `LISTEN` after every reconnect, and a replay query.

**postgres.js (`postgres@3.4.9`) implements exactly this natively:**
...
const { unlisten } = await sql.listen(
  ...
  (payload) => send('message', JSON.parse(payload)),
  () => replayMissedMessages(),   // onlisten: fires on connect AND every reconnect
)
```
...
**Recommendation:** use `postgres.js` for both the query pool and the listener. If you keep `pg`, you must write the reconnect/replay yourself — budget a half-day and a test for it.

Additional NOTIFY gotchas regardless of driver:
...
- **Subscription lifecycle / 410 handling** — the exact contract:
  - `sendNotification()` rejects with an error carrying `statusCode`.
  - **`410 Gone`** = subscription permanently expired/unsubscribed → **delete the row**.
  - **`404 Not Found`** = same treatment, delete.
  - **`429`** = push service rate limit → retry with backoff, honour `Retry-After`.
  - **`413`** = payload too large. Keep encrypted payloads **under 4 KB**; practically, send `{ conversationId, titleKey, bodyKey }` and localise in the service worker, never the message text.
  - `5xx` = transient, retry.
...
**If the Swahili spike fails (PROJECT.md open risk):**
...
**If Traefik proves hostile to long-lived SSE on the target Coolify host:**
...
**If a second responder is added later (explicitly non-blocking requirement):**
...
---
*Stack research for: self-hosted anonymous 1:1 chat PWA with mandatory web push and LLM translation*
*Researched: 2026-07-20*
